...
This action provides xref:managing/configuration/cass_yaml_file.adoc[configurable durability] by logging every write made to a {cassandra} node.
...
An SSTable consists of several components, each stored in separate files:
