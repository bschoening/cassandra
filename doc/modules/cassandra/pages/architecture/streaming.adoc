...
connections are opened simultaneously. Non-blocking means that threads remain
active and do not pause, as they do not wait for a response after sending a request.
A response might be returned by a different thread. In an asynchronous setup,
connections and threads are independent and not directly linked in a 1:1 manner.
This allows for the possibility of having many more connections open than there are threads.
